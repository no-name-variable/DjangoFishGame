...
from apps.fishing.services.time_service import get_time_of_day_modifier


def select_fish(location, rod_setup):
    """
    ...
    for lf in location_fish:
        ...
        weight *= get_time_of_day_modifier(fish)
    ...
    rarity_val = get_potion_effect_value(rod_setup.player, 'rarity')
    if rarity_val:
...
def generate_fish_weight(species, player=None):
    """
...
def generate_fish_length(species, weight):
