...
def check_record(player, species, weight, length, location):
    """
...
def check_achievements(player):
    ...
    for achievement in Achievement.objects.exclude(pk__in=existing):
        if _check_condition(player, achievement):
...
def _check_condition(player, achievement):
    ...
    if ct == 'fish_count':
        ...
        from django.db.models import Sum # TODO: Хуйня переделывай
        ...
        from apps.quests.models import PlayerQuest # TODO: Хуйня переделывай
