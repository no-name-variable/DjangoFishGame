...
def finalize_tournament(tournament_id):
    ...
    if tournament.tournament_type == Tournament.TournamentType.TEAM:
        _finalize_team_tournament(tournament, entries)
    else:
        _finalize_individual_tournament(tournament)
...
def _finalize_individual_tournament(tournament):
    ...
    _award_prizes(tournament, ranked_entries[:3])


def _finalize_team_tournament(tournament, entries):
...
def _award_prizes(tournament, top_entries):
